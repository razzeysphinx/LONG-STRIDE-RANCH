...
3. Rebuilt `PageHero` to support image-led and quiet variants.
...
- Existing responsive navigation and mobile inquiry behavior are retained.
...
- `npm.cmd run build` — passed after allowing the existing `next/font/google` imports to fetch Inter and Cormorant Garamond.
